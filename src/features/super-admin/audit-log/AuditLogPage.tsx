...
export function AuditLogPage() {
  ...
  const actions = (params.get('actions')?.split(',').filter(Boolean) ?? []) as string[];
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await downloadCsv('/exports/audit-log.csv', `audit-log-${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const columns: GridColDef<ApiAuditEntry>[] = [
    {
      ...
      minWidth: 200,
      renderCell: ({ row }) => (
        <Typography sx={{ fontSize: 13, fontFamily: 'ui-monospace, monospace' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Avatar
            sx={{
              width: 28,
              height: 28,
              bgcolor: row.actorRole === 'anonymous' ? '#8C8C8C' : 'primary.main',
              fontSize: 11,
            }}
          >
            {row.actorName
          ...
          <Box>
            <Typography sx={{ fontSize: 13, fontWeight: 500 }}>{row.actorName}</Typography>
            <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>{row.actorRole}</Typography>
          </Box>
        </Box>
      ),
    },
    {
      ...
      flex: 1,
      minWidth: 200,
      renderCell: ({ row }) => <StatusChip label={row.action} tone={actionTone(row.action)} />,
    },
    {
      field: 'target',
      headerName: 'Target',
      ...
      minWidth: 220,
      renderCell: ({ row }) => {
        const Icon = (row.entityType ? TARGET_ICONS[row.entityType] : undefined) ?? AssignmentIcon;
        return (
    ...
    },
    {
      ...
      minWidth: 100,
      renderCell: ({ row }) => (
        <Typography sx={{ fontSize: 12, fontFamily: 'ui-monospace, monospace', color: 'text.secondary' }}>
          {row.ip ?? '—'}
        </Typography>
      ),
    },
  ...
  return (
    <>
      ...
      <Card sx={{ p: 2, mb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap', mb: 1 }}>
          ...
          <Autocomplete
            ...
            value={actions}
            onChange={(_, value) => setParam('actions', value.length ? value.join(',') : null)}
        ...
        <Accordion elevation={0} disableGutters sx={{ '&:before': { display: 'none' }, bgcolor: 'transparent' }}>
          ...
          <AccordionDetails sx={{ px: 0, pt: 0, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <TextField
              size="small"
              label="IP address"
              value={ipF}
      ...
      {isLoading ? (
        <Card sx={{ p: 2 }}>
          {[...Array(8)].map((_, i) => <Skeleton key={i} height={56} sx={{ mb: 1 }} />)}
        </Card>
      ) : (
