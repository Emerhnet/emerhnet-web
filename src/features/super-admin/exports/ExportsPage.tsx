...
const DATASETS: DatasetCard[] = [
  {
    key: 'hospitals',
    title: 'Hospitals',
    description: 'All hospitals on the platform with profile fields and status.',
...
export function ExportsPage() {
  ...
  const exportNow = async (d: DatasetCard) => {
    setBusy(d.key);
    try {
      await downloadCsv(d.path, `${d.filename}-${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <>
      <PageHeader title="Exports" subtitle="Download platform data as CSV files." />
      ...
      <div className="grid grid-cols-3 gap-4">
        {DATASETS.map((d) => {
          ...
          return (
            <Card key={d.key} sx={{ p: 2.5, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <Typography sx={{ fontSize: 16, fontWeight: 600 }}>{d.title}</Typography>
              ...
              <Button
                ...
                disabled={isBusy}
                sx={{ height: 40, fontWeight: 600, alignSelf: 'flex-start', mt: 0.5 }}
              >
