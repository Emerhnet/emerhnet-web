...
import { StatusChip, CATEGORY_TONE, type StatusTone } from '@/shared/components/StatusChip';
...
const CHECKLIST: ChecklistItem[] = [
  { code: 'B1', text: 'Hospital identity exists in the public NIN registry or state list' },
  { code: 'B2', text: 'Documents are authentic on inspection (signatures, stamps, dates consistent)' },
  { code: 'B3', text: 'Authorisation letter is valid and admin is reachable' },
  { code: 'B4', text: "Phone callback completed to the hospital's published number" },
  { code: 'B5', text: 'No conflicting registration in the system' },
];

const SCAN_TONE: Record<string, StatusTone> = { Clean: 'success', Scanning: 'warning', Quarantined: 'danger' };
...
function DocumentActions({ hospitalId, slotKey }: { hospitalId: string; slotKey: string }) {
  const [enabled, setEnabled] = useState(false);
  const [pendingMode, setPendingMode] = useState<'preview' | 'download' | null>(null);
  const { data: url, isFetching } = useHospitalDocumentUrl(hospitalId, slotKey, enabled);
  ...
  return (
    <>
      <Tooltip title="Preview">
        <span>
          <IconButton size="small" aria-label="Preview" onClick={() => handleClick('preview')} disabled={isFetching}>
      ...
      <Tooltip title="Download">
        <span>
          <IconButton size="small" aria-label="Download" onClick={() => handleClick('download')} disabled={isFetching}>
...
export function ReviewPage() {
  ...
  const [credentials, setCredentials] = useState<
    { email: string; tempPassword: string; emailSent: boolean } | null
  >(null);

  const initialChecks = useMemo(
    () => Object.fromEntries(CHECKLIST.map((c) => [c.code, { checked: false, notes: '' }])),
    [],
  );
  const [checks, setChecks] = useState(initialChecks);

  const applicableCount = CHECKLIST.length;
  const completedCount = CHECKLIST.filter((c) => checks[c.code]?.checked).length;
  const allDone =
  ...
  const handleApprove = async (notes: string) => {
    try {
      ...
      enqueueSnackbar(`${hospital?.hospitalName ?? 'Hospital'} approved.`, { variant: 'success' });
      setCredentials({
  ...
  const handleReject = async (notes: string) => {
    try {
      ...
      setRejectOpen(false);
      enqueueSnackbar(`${hospital?.hospitalName ?? 'Hospital'} rejected.`, { variant: 'success' });
      navigate('/admin/pending');
    } catch (err) {
      enqueueSnackbar(getApiErrorMessage(err), { variant: 'error' });
    }
  ...
  if (isLoading) {
    return (
      <>
        <Breadcrumbs items={[{ label: 'Home', to: '/admin/dashboard' }, { label: 'Pending Registrations', to: '/admin/pending' }, { label: '…' }]} />
  ...
  if (isError || !hospital) {
    return (
      <>
        <Breadcrumbs items={[{ label: 'Home', to: '/admin/dashboard' }, { label: 'Pending Registrations', to: '/admin/pending' }, { label: 'Error' }]} />
        <Alert severity="error" sx={{ mt: 2 }}>Failed to load registration details.</Alert>
      </>
    );
  }
    ...
    <>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          ...
          <Tooltip title={allDone ? '' : 'Complete the verification checklist to enable.'}>
            <span>
              <Button
                variant="contained"
        ...
        <Box sx={{ gridColumn: 'span 3' }}>
          <Card>
            <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ px: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
              ...
              <Tab label="Address" sx={{ textTransform: 'none', fontWeight: 600 }} />
              <Tab label="Admin Contact" sx={{ textTransform: 'none', fontWeight: 600 }} />
            </Tabs>
            <Box sx={{ p: 3 }}>
              {tab === 0 && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <ReadRow label="Clinical Establishments Act Licence" value={hospital.ceaLicenceNumber || '—'} />
                  <ReadRow label="Category" value={hospital.category} />
                  <ReadRow label="CGHS Empanelment" value={hospital.cghsEmpanelment ? 'Yes' : 'No'} />
                  <ReadRow label="Ayushman Bharat Empanelment" value={hospital.ayushmanEmpanelment ? 'Yes' : 'No'} />
                  <ReadRow label="Submitted" value={formatIst(hospital.createdAt)} />
                  <div>
                    <Typography variant="caption" sx={{ color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 500, display: 'block' }}>
                      Tracking ID
                    </Typography>
                    <Typography sx={{ mt: 0.5, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: 14, fontWeight: 500 }}>
                      {hospital.trackingId}
                    </Typography>
                  </div>
                </div>
              ...
              {tab === 1 && (
                <div className="grid grid-cols-2 gap-4">
                  <ReadRow label="Address line 1" value={hospital.address.line1} />
                  <ReadRow label="Address line 2" value={hospital.address.line2 || '—'} />
                  ...
                  <ReadRow label="Coordinates" value={`${hospital.address.latitude}, ${hospital.address.longitude}`} />
                </div>
              )}
              {tab === 2 && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <ReadRow label="Admin name" value={hospital.adminContact.name} />
                  <ReadRow label="Admin email" value={hospital.adminContact.email} />
                  <ReadRow label="Admin phone" value={hospital.adminContact.phone} />
                </div>
              )}
            </Box>
          </Card>
          ...
          <SectionCard title="Documents" bodyPadding={0}>
            <Box>
              {hospital.documents.length === 0 ? (
                <Typography variant="body2" sx={{ p: 3, color: 'text.secondary' }}>No documents uploaded.</Typography>
              ) : (
                hospital.documents.map((d, i) => (
                  <Box key={d.slotKey}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2.5, py: 1.5 }}>
                      <Box sx={{ width: 36, height: 36, borderRadius: 1, bgcolor: '#F2EEE6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <DescriptionIcon sx={{ fontSize: 20, color: 'primary.main' }} />
                      </Box>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                          <Typography sx={{ fontSize: 14, fontWeight: 600 }}>{SLOT_LABELS[d.slotKey] ?? d.slotKey}</Typography>
                          <StatusChip label="Mandatory" tone={REQ_TONE[d.slotKey] ?? 'muted'} />
                        </Box>
                        <Typography sx={{ fontSize: 12, color: 'text.secondary', mt: 0.25 }}>
                          {d.fileName} · {formatBytes(d.sizeBytes)}
                        </Typography>
                        <Box sx={{ mt: 0.75 }}>
                          <StatusChip label="Scan: Clean" tone={SCAN_TONE['Clean']} />
                        </Box>
                      </Box>
        ...
        action={
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 220 }}>
            ...
            <LinearProgress
              ...
              sx={{ flex: 1, height: 6, borderRadius: 999, bgcolor: '#E5E7EB', '& .MuiLinearProgress-bar': { bgcolor: 'primary.main', borderRadius: 999 } }}
            />
          </Box>
        }
        bodyPadding={0}
      >
        <Box>
          {CHECKLIST.map((c, i) => {
            ...
            return (
              <Box key={c.code}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '48px 1fr 32px 1fr', alignItems: 'center', gap: 2, px: 3, py: 1.75 }}>
                  <Typography sx={{ fontSize: 13, fontWeight: 600, color: 'text.secondary' }}>{c.code}</Typography>
                  <Typography sx={{ fontSize: 14 }}>{c.text}</Typography>
                  <Checkbox
                    checked={item?.checked ?? false}
                    onChange={(e) => setChecks((s) => ({ ...s, [c.code]: { ...s[c.code], checked: e.target.checked } }))}
                  />
                  <TextField
                    size="small"
                    placeholder="Notes"
                    value={item?.notes ?? ''}
                    onChange={(e) => setChecks((s) => ({ ...s, [c.code]: { ...s[c.code], notes: e.target.value } }))}
                    error={item?.checked && !item.notes.trim()}
                    helperText={item?.checked && !item.notes.trim() ? 'Notes required' : ''}
                  />
                </Box>
