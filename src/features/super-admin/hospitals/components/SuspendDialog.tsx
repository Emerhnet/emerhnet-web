...
export function SuspendDialog({
  ...
  return (
    <Dialog open={open} onClose={onCancel} maxWidth="xs" fullWidth PaperProps={{ sx: { width: 480 } }}>
      <DialogTitle sx={{ fontSize: 18, fontWeight: 600 }}>Suspend {hospitalName}?</DialogTitle>
      <DialogContent>
        <Typography variant="body2" sx={{ mb: 2 }}>
          While suspended, the Hospital Admin will be unable to sign in. All hospital data is
          preserved and will be available again upon reinstatement. The hospital and its
        ...
        >
          ...
          <Box sx={{ color: '#842029' }}>
            ...
            <Box component="ul" sx={{ pl: 2.5, m: 0, '& li': { fontSize: 13, mb: 0.25 } }}>
              <li>All active hospital login sessions will be terminated.</li>
              <li>The hospital will no longer appear in search results for citizens.</li>
        ...
        <Box sx={{ mb: 0.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography component="label" htmlFor="suspendReason" variant="body2" sx={{ fontWeight: 500 }}>
            Reason for suspension <span style={{ color: '#842029' }}>*</span>
          </Typography>
          <Typography variant="caption" sx={{ color: len > MAX_LEN ? 'error.main' : 'text.secondary' }}>
            {len} / {MAX_LEN}
          </Typography>
        </Box>
        <TextField
