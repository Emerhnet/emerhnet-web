...
const STATUSES: ApiHospitalStatus[] = [
  "approved",
  "pending",
  "suspended",
  "rejected",
...
export function HospitalsListPage() {
  ...
  const statusF =
    (params.get("status") as ApiHospitalStatus | null) ?? undefined;
  const categoryF =
    (params.get("category") as "Government" | "Private" | "Trust" | null) ??
    undefined;
  ...
  const hasFilters = Boolean(
    q || statusF || categoryF || stateF || cghs || ayushman,
  );
  const resetFilters = () =>
    ...
    <>
      <PageHeader
        ...
        action={
          <Button
            ...
            startIcon={<SendIcon />}
            onClick={() =>
              enqueueSnackbar("Send Invitation — coming soon.", {
                variant: "info",
              })
            }
      ...
      >
        ...
        <TextField
          ...
          value={stateF ?? "all"}
          onChange={(e) =>
            setParam("state", e.target.value === "all" ? null : e.target.value)
          }
          sx={{ width: 180 }}
        >
        ...
        <FormControlLabel
          control={
            <Switch
              size="small"
              checked={cghs}
              onChange={(e) => setParam("cghs", e.target.checked ? "1" : null)}
            />
          }
        ...
        <FormControlLabel
          control={
            <Switch
              ...
              checked={ayushman}
              onChange={(e) =>
                setParam("ayushman", e.target.checked ? "1" : null)
              }
            />
          }
          label={
            <Typography sx={{ fontSize: 13 }}>Ayushman Bharat only</Typography>
          }
        />
        <Box sx={{ flex: 1 }} />
        {hasFilters && (
