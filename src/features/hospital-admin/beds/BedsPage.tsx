...
function OccupancyBar({ occupied, total }: { occupied: number; total: number }) {
  ...
  const tone: 'normal' | 'warning' | 'danger' = pct > 90 ? 'danger' : pct >= 70 ? 'warning' : 'normal';
  const color = tone === 'danger' ? '#842029' : tone === 'warning' ? '#8B5A00' : '#0F5132';
  const trackColor = tone === 'danger' ? '#F8D7DA' : tone === 'warning' ? '#FFF3CD' : '#E5E7EB';
  return (
    <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', gap: 1 }}>
      <Box sx={{ flex: 1, height: 8, bgcolor: trackColor, borderRadius: 999, overflow: 'hidden' }}>
        <Box
          sx={{
      ...
      <Typography sx={{ fontSize: 12, fontWeight: 600, color, minWidth: 36, textAlign: 'right' }}>
        {Math.round(pct)}%
      </Typography>
    </Box>
  );
...
const SUMMARY_TONE: Record<SummaryTone, { bg: string; iconColor: string; valueColor: string }> = {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Typography sx={{ fontSize: 36, fontWeight: 700, lineHeight: 1.1, color: t.valueColor, mt: 0.25 }}>
          {value}
        </Typography>
        {hint && (
  ...
  return (
    <>
      ...
      <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={() => setAnchor(null)}>
        <MenuItem onClick={() => { setAnchor(null); onEdit(); }}>Edit counts</MenuItem>
...
export function BedsPage() {
  ...
  const handleCreate = async (p: { type: string; total: number; occupied: number }) => {
    try {
      await createMutation.mutateAsync(p);
      enqueueSnackbar(`Added "${p.type}" with ${p.total} beds.`, { variant: 'success' });
      setConfigOpen(false);
    } catch (err) {
  ...
  const handleUpdate = async (p: { total: number; occupied: number }) => {
    ...
    try {
      ...
      enqueueSnackbar(`Updated ${editing.type} counts.`, { variant: 'success' });
      setEditing(null);
    } catch (err) {
      enqueueSnackbar(getApiErrorMessage(err), { variant: 'error' });
    }
  ...
  const columns: GridColDef<ApiBed>[] = [
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography sx={{ fontSize: 14, fontWeight: 600 }}>{row.type}</Typography>
        </Box>
      ),
    },
    {
      ...
      headerAlign: 'left',
      renderCell: ({ row }) => (
        <Typography sx={{ fontSize: 14, fontWeight: 500 }}>{row.total}</Typography>
      ),
    },
    {
      field: 'occupied',
      ...
      headerAlign: 'left',
      renderCell: ({ row }) => (
        <Typography sx={{ fontSize: 14, color: 'text.secondary' }}>{row.occupied}</Typography>
      ),
    },
    {
      field: 'available',
      ...
      headerAlign: 'left',
      sortable: false,
      renderCell: ({ row }) => <AvailablePill value={row.total - row.occupied} />,
    },
    {
      field: 'occupancy',
      ...
      minWidth: 160,
      sortable: false,
      renderCell: ({ row }) => <OccupancyBar occupied={row.occupied} total={row.total} />,
    },
    {
      field: 'updatedAt',
    ...
    },
    {
      ...
      headerAlign: 'right',
      renderCell: ({ row }) => (
        <RowMenu onEdit={() => setEditing(row)} onDelete={() => void handleDelete(row)} />
      ),
    },
  ];
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        <SummaryCard label="Total Beds" value={total} Icon={HotelIcon} tone="primary" hint={`Across ${rows.length} bed types`} />
        <SummaryCard label="Currently Occupied" value={occupied} Icon={PeopleAltIcon} tone="info" hint={total > 0 ? `${Math.round((occupied / total) * 100)}% utilization` : ''} />
        <SummaryCard label="Available" value={available} Icon={CheckCircleOutlineIcon} tone="success" hint="Ready for admission" />
      </Box>
      ...
      {isLoading ? (
        <Card sx={{ p: 2 }}>
          {[...Array(5)].map((_, i) => <Skeleton key={i} height={64} sx={{ mb: 1 }} />)}
        </Card>
      ) : (
