...
export function RequestChangeDialog({
  ...
  initialField?: Field;
  onCancel: () => void;
  onConfirm: (payload: { field: Field; proposed: string; reason: string; fileName?: string }) => void;
}) {
  ...
  const valid = useMemo(
    () => proposed.trim().length > 0 && proposed.trim() !== current && reason.trim().length > 0,
  ...
  return (
    <Dialog open={open} onClose={onCancel} maxWidth="xs" fullWidth PaperProps={{ sx: { width: 520 } }}>
      <DialogTitle sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <Box>
          <Typography sx={{ fontSize: 18, fontWeight: 600 }}>Request a profile change</Typography>
          <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
            Changes to identity fields require Galas review before they take effect.
          </Typography>
        </Box>
      ...
      <DialogContent dividers>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <TextField
            ...
            placeholder={field === 'Geo-coordinates' ? 'e.g. 18.5241, 73.8602' : ''}
          />
        </div>
        ...
        <Typography sx={{ fontSize: 14, fontWeight: 500, mb: 1 }}>Supporting document</Typography>
        <Box
          component="label"
          sx={{
            display: 'flex',
        ...
        >
          ...
          />
          <Box sx={{ width: 40, height: 40, borderRadius: '50%', bgcolor: '#E8EEF5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
