...
function actionTone(action: string): StatusTone {
  ...
  if (action.endsWith('.rejected') || action.endsWith('.deleted') || action.endsWith('.suspended') || action.endsWith('.deactivated') || action.endsWith('.cancelled')) return 'danger';
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          >
            {row.actorName.split(' ').map((n) => n[0]).slice(0, 2).join('').toUpperCase() || 'SY'}
          </Avatar>
          <Box>
