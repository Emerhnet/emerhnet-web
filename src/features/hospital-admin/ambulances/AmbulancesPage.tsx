...
const STATUS_KEYS: AmbulanceStatus[] = ['Available', 'On Duty', 'Under Maintenance', 'Out of Service'];

const PILL_TONE: Record<AmbulanceStatus, { bg: string; fg: string; rail: string; Icon: typeof CheckCircleIcon }> = {
...
function StatusPill({ label, count }: { label: AmbulanceStatus; count: number }) {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Typography sx={{ fontSize: 28, fontWeight: 600, lineHeight: 1.1, color: dim ? 'text.disabled' : t.fg, mt: 0.25 }}>
  ...
  const handleSubmit = async (input: CreateAmbulanceInput) => {
    try {
      if (drawerMode === 'edit' && selected) {
        ...
        enqueueSnackbar(`Updated ${input.vehicleNumber}.`, { variant: 'success' });
      } else {
        await createMutation.mutateAsync(input);
        enqueueSnackbar(`Registered ${input.vehicleNumber}.`, { variant: 'success' });
      }
      setDrawerMode(null);
      setSelected(null);
    } catch (err) {
  ...
  const columns: GridColDef<ApiAmbulance>[] = [
    {
      ...
      minWidth: 160,
      renderCell: ({ row }) => (
        <Typography sx={{ fontSize: 14, fontWeight: 600, fontFamily: 'ui-monospace, monospace', color: 'primary.main' }}>
          {row.vehicleNumber}
        </Typography>
      ),
    },
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box>
          <Typography sx={{ fontSize: 14, fontWeight: 500 }}>{row.driverName}</Typography>
          <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>{row.driverPhone}</Typography>
        </Box>
      ),
    },
    {
        ...
        return (
          <Tooltip title={row.equipment.join(', ')}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'wrap' }}>
              {visible.map((e) => (
                <Chip key={e} size="small" label={e} sx={{ bgcolor: '#F2EEE6', fontSize: 11, height: 22 }} />
              ))}
              {rest > 0 && (
                <Chip
                  size="small"
                  label={`+${rest} more`}
                  sx={{ bgcolor: '#E8EEF5', color: 'primary.main', fontSize: 11, height: 22, fontWeight: 500 }}
                />
              )}
            </Box>
          </Tooltip>
    ...
    },
    {
      ...
      flex: 0.7,
      minWidth: 130,
      renderCell: ({ row }) => <StatusChip label={row.status} tone={STATUS_TONE[row.status]} />,
    },
    {
      field: 'updatedAt',
