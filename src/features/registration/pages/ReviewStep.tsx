...
export function ReviewStep() {
  ...
  const { hospitalDetails, addressLocation, adminContact, documents, invite } = useRegistrationStore();
  ...
  const onSubmit = async () => {
    try {
      ...
      navigate('/register-hospital/submitted', {
        state: { trackingId: result.trackingId, adminEmail: adminContact.adminEmail },
      });
    } catch (err) {
      enqueueSnackbar(getApiErrorMessage(err, 'Submission failed. Try again.'), { variant: 'error' });
    }
  };
  ...
  return (
    <Box>
      ...
      <Accordion defaultExpanded disableGutters elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, mb: 1.5, '&:before': { display: 'none' } }}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ bgcolor: '#F2EEE6' }}>
        ...
        <AccordionDetails>
          <div className="grid grid-cols-2 gap-4">
            <ReadRow label="Hospital name" value={hospitalDetails.hospitalName} />
            <ReadRow label="NIN" value={hospitalDetails.nin} />
            <ReadRow label="Category" value={hospitalDetails.category} />
            <ReadRow label="CEA licence" value={hospitalDetails.ceaLicenceNumber} />
            <ReadRow label="CGHS empanelment" value={hospitalDetails.cghsEmpanelment} />
            <ReadRow label="Ayushman Bharat" value={hospitalDetails.ayushmanEmpanelment} />
          </div>
        </AccordionDetails>
      </Accordion>

      <Accordion disableGutters elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, mb: 1.5, '&:before': { display: 'none' } }}>
        ...
        <AccordionDetails>
          <div className="grid grid-cols-2 gap-4">
            <ReadRow label="Address line 1" value={addressLocation.addressLine1} />
            <ReadRow label="Address line 2" value={addressLocation.addressLine2} />
      ...
      <Accordion disableGutters elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, mb: 1.5, '&:before': { display: 'none' } }}>
        ...
        <AccordionDetails>
          <div className="grid grid-cols-2 gap-4">
            <ReadRow label="Hospital email" value={adminContact.hospitalEmail} />
            <ReadRow label="Hospital phone" value={adminContact.hospitalPhone} />
        ...
        <FormControlLabel
          ...
          label={
            <Typography variant="body2">
              I confirm the information provided is accurate and that I have authority to register
              this hospital.
            </Typography>
          }
