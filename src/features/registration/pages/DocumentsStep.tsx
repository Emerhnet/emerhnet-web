...
const SLOTS: SlotDef[] = [
  {
    ...
    description: 'Primary registration document issued by the relevant authority',
    ...
    return (
      <Box
        sx={{
          position: 'relative',
          border: '1px solid',
          borderColor: isError ? 'error.light' : isClean ? 'divider' : 'primary.light',
      ...
      >
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <div className="flex items-center gap-2 flex-wrap">
            <Typography sx={{ fontSize: 16, fontWeight: 600 }}>{slot.name}</Typography>
            <RequirementBadge value={slot.requirement} />
          </div>
          <Typography variant="body2" sx={{ color: isError ? 'error.main' : 'text.secondary', mt: 0.25 }}>
            {isScanning
              ? 'Uploading…'
              : isError
        ...
        {isScanning && (
          <AutorenewIcon sx={{ color: 'primary.main', animation: 'spin 1.4s linear infinite' }} />
        )}
        {isError && (
          <IconButton size="small" onClick={onPickFile} aria-label="Retry upload">
...
export function DocumentsStep() {
  ...
  const handleFile = async (key: DocumentSlotKey, file: File) => {
    setDocument(key, { fileName: file.name, sizeBytes: file.size, scanStatus: 'scanning' });
    try {
      ...
      setDocument(key, { fileName: file.name, sizeBytes: file.size, scanStatus: 'error' });
      enqueueSnackbar(getApiErrorMessage(err, 'Upload failed. Please try again.'), { variant: 'error' });
    }
  };

  const mandatoryUploaded = SLOTS.filter((s) => s.requirement === 'Mandatory').every(
    (s) => documents[s.key]?.scanStatus === 'clean' && documents[s.key]?.s3Key,
  );

  const anyUploading = Object.values(documents).some((d) => d?.scanStatus === 'scanning');
    ...
    <Box>
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
          Upload supporting documents. PDFs and images are accepted, up to 10 MB each.
        </Typography>
      </Box>
